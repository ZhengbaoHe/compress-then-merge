...
def evaluate_logits(model, loader, device, mask_class=None, eval=True, silent=False):
...
def get_clip_features(
    ...
    with torch.no_grad(), autocast():
        ...
        for inputs, labels in tqdm(loader, 'Extracting features', disable=silent):
            ...
            all_encodings.append(normed_encodings.to(device))
...
def prepare_data(config, device=DEFAULT_DEVICE):
    ...
    print(f"Preparing data for {data_config['type']} dataset.")
    # NLI datasets
...
def prepare_llama(config, device):
    ...
    print("memory before loading ", process.memory_info().rss / 1024**2, "model name: ", model_name_or_path)
    if "8B" in model_name_or_path:
        ptm_model = AutoModelForSequenceClassification.from_pretrained(
            model_name_or_path, return_dict=True, cache_dir=config['cachedir'], num_labels=3, torch_dtype=torch.float16)
    else:
        ptm_model = AutoModelForSequenceClassification.from_pretrained(
    ...
    for base_path in tqdm(config['bases'], desc="Preparing Models"):
        if base_path.endswith('.pt'):
            ...
            lora_state_dict = torch.load(base_path, map_location=DEFAULT_DEVICE)
...
def get_model_from_sd(state_dict, base_model):
    ...
    if torch.cuda.is_available():
        ...
        devices = [x for x in range(torch.npu.device_count())]
...
def prepare_models(config, device=DEFAULT_DEVICE):
...
def parse_eval_args():
    ...
    parser.add_argument('--merge_space', type=str, help='Merge space', choices=('full', 'knots', 'core', "low_rank", "low_rank_core", "lego", "robustmerge"))
    ...
    parser.add_argument('--low_rank', action='store_true', default=False,
                        help='Whether to use low rank approximation')
    parser.add_argument('--lora_rank', type=int, default=-1, help='Rank for LoRA models')
    ...
    parser.add_argument('--cart_scaling_coeffs', type=float, default=1.0)
    
    parser.add_argument('--only_eval', action='store_true', default=False)
    parser.add_argument('--seed', type=int, default=420)
    parser.add_argument('--search_beta_first', action='store_true', default=False)
...
def load_model(model, save_path, model_device=DEFAULT_DEVICE):
...
def get_clip_encodings(path, device):
...
def get_strides(config, rounds=1):
    strides = {}
    for key, values in config.items():
        T = len(values)
        strides[key] = int( (T/2)**((3-rounds)/3) ) + 1
    return strides